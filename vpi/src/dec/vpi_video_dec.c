...
void *decode_process(void *param)
{
    ...
    while (!vpi_ctx->dec_thread_finish) {
        ...
        case Dec_H264_H10P:
            VPILOGD("start h264 dec process\n");
            ret = vpi_decode_h264_dec_process(vpi_ctx);
            VPILOGD("finish h264 dec process\n");
            break;
        case Dec_HEVC:
            VPILOGD("start hevc dec process\n");
            ret = vpi_decode_hevc_dec_process(vpi_ctx);
            VPILOGD("finish hevc dec process\n");
            break;
        case Dec_VP9:
            VPILOGD("start vp9 dec process\n");
            ret = vpi_decode_vp9_dec_process(vpi_ctx);
            VPILOGD("finish vp9 dec process\n");
            break;
        default:
            break;
        }
...
static VpiRet vpi_dec_init_decoder(VpiDecCtx *vpi_ctx, void *cfg)
{
    ...
    vpi_ctx->use_extra_buffers_num = 15;
    ...
    //pthread_cond_init(&vpi_ctx->strm_buffer_ctrl_cond, NULL);
    //pthread_cond_init(&vpi_ctx->dec_frame_finish_cond, NULL);
...
VpiRet vpi_vdec_close(VpiDecCtx *vpi_ctx)
{
    ...
    pthread_mutex_destroy(&vpi_ctx->ext_buffer_ctrl);

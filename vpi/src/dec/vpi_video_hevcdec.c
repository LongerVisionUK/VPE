...
{
    enum DecRet rv;
    uint32_t stride, stride_ch, i, bit_depth;
    struct HevcDecPicture hpic;
    uint32_t *tile_status_virtual_address = NULL;
...
#ifdef USE_EXTERNAL_BUFFER
VpiRet vpi_dec_hevc_get_buffer_info(VpiDecInst inst,
...
#endif
    if (rv) {
        VPILOGD("HevcDecGetBufferInfo ret %s\n", dec_ret_string(rv));
        return VPI_ERR_UNKNOWN;
    }
    return VPI_SUCCESS;
}

VpiRet vpi_dec_hevc_add_buffer(VpiDecInst inst, struct DWLLinearMem *buf)
{
    enum DecRet rv;

    rv = HevcDecAddBuffer(inst, buf);
    if (rv) {
        VPILOGD("HevcDecAddBuffer ret %s\n", dec_ret_string(rv));
        return VPI_ERR_UNKNOWN;
    }
    return VPI_SUCCESS;
}
...
int vpi_decode_hevc_dec_frame(VpiDecCtx *vpi_ctx, void *indata, void *outdata)
{
    ...
    do {
        ...
        case DEC_WAITING_FOR_BUFFER:
            ...
            dec_ret =
                HevcDecGetBufferInfo(vpi_ctx->dec_inst, &vpi_ctx->hevc_hbuf);
            VPILOGD("HevcDecGetBufferInfo ret %d\n", dec_ret);
            VPILOGD("buf_to_free %p, next_buf_size %d, buf_num %d\n",
                    (void *)vpi_ctx->hevc_hbuf.buf_to_free.virtual_address,
                    vpi_ctx->hevc_hbuf.next_buf_size,
                    vpi_ctx->hevc_hbuf.buf_num);

            /* For HEVC, all external buffers will be freed
                 * after new headers comes. */
            if (dec_ret == DEC_WAITING_FOR_BUFFER &&
                vpi_ctx->hevc_hbuf.buf_to_free.bus_address) {
                DWLFreeLinear(vpi_ctx->dwl_inst,
                              &vpi_ctx->hevc_hbuf.buf_to_free);
                uint32_t id;
                id = vpi_dec_find_ext_buffer_index(vpi_ctx,
                                                   (uint32_t
                                                        *)(vpi_ctx->hevc_hbuf
                                                               .buf_to_free
                                                               .bus_address));
            ...
            if (vpi_ctx->hevc_hbuf.next_buf_size) {
                /* Only add minimum required buffers at first. */
                //extra_buffer_num = hbuf.buf_num - fb_dec_ctx->min_buffer_num;
                vpi_ctx->buffer_size = vpi_ctx->hevc_hbuf.next_buf_size;
#ifdef ALWAYS_OUTPUT_REF
                alloc_buffer_num = vpi_ctx->hevc_hbuf.buf_num;
#else
                alloc_buffer_num =
                    vpi_ctx->hevc_hbuf.buf_num + vpi_ctx->use_extra_buffers_num;
#endif
                for (int i = 0; i < alloc_buffer_num; i++) {
                    mem.mem_type = DWL_MEM_TYPE_DPB;
                    VPILOGD("DWLMallocLinear %d DWL_MEM_TYPE_DPB size=%d\n", i,
                            vpi_ctx->hevc_hbuf.next_buf_size);
                    ret =
                        DWLMallocLinear(vpi_ctx->dwl_inst,
                                        vpi_ctx->hevc_hbuf.next_buf_size, &mem);
                    if (ret) {
                        vpi_ctx->num_buffers = i;
                        return -1;
                    }
                    dec_ret = HevcDecAddBuffer(vpi_ctx->dec_inst, &mem);

                    uint32_t id = vpi_dec_find_empty_index(vpi_ctx);
                ...
                //num_buffers = vpi_ctx->min_buffer_num;
                if (vpi_ctx->num_buffers >= vpi_ctx->min_buffer_num) {

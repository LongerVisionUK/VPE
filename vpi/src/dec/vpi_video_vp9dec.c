...
int vpi_decode_vp9_dec_frame(VpiDecCtx *vpi_ctx, void *indata, void *outdata)
{
    ...
    do {
        ...
        case DEC_WAITING_FOR_BUFFER: {
            ...
            ret = Vp9DecGetBufferInfo(vpi_ctx->dec_inst, &vpi_ctx->vp9_hbuf);
            VPILOGD("Vp9DecGetBufferInfo ret %d\n", ret);
            VPILOGD("buf_to_free %p, next_buf_size %d, buf_num %d\n",
                    (void *)vpi_ctx->vp9_hbuf.buf_to_free.virtual_address,
                    vpi_ctx->vp9_hbuf.next_buf_size, vpi_ctx->vp9_hbuf.buf_num);
            ...
            if (ret == DEC_WAITING_FOR_BUFFER &&
                vpi_ctx->vp9_hbuf.buf_to_free.bus_address) {
                DWLFreeLinear(vpi_ctx->dwl_inst,
                              &vpi_ctx->vp9_hbuf.buf_to_free);
                uint32_t id =
                    vpi_dec_find_ext_buffer_index(vpi_ctx,
                                                  (uint32_t *)vpi_ctx->vp9_hbuf
                                                      .buf_to_free.bus_address);
            ...
            if (vpi_ctx->vp9_hbuf.next_buf_size) {
                /* Only add minimum required buffers at first. */
                //extra_buffer_num = vpi_ctx->vp9_hbuf.buf_num - vpi_ctx->min_buffer_num;
                vpi_ctx->buffer_size = vpi_ctx->vp9_hbuf.next_buf_size;
#ifdef ALWAYS_OUTPUT_REF
                alloc_buffer_num = vpi_ctx->vp9_hbuf.buf_num;
#else
                alloc_buffer_num =
                    vpi_ctx->vp9_hbuf.buf_num + vpi_ctx->use_extra_buffers_num;
#endif
                VPILOGD("use_extra_buffers_num %d\n",
                ...
                for (int i = 0; i < alloc_buffer_num; i++) {
                    ...
                    VPILOGD("DWLMallocLinear %d DWL_MEM_TYPE_DPB size = %d\n",
                            i, vpi_ctx->vp9_hbuf.next_buf_size);
                    ret =
                        DWLMallocLinear(vpi_ctx->dwl_inst,
                                        vpi_ctx->vp9_hbuf.next_buf_size, &mem);
                    if (ret) {
                        vpi_ctx->num_buffers = i;
                        return -1;
                    }
                    ret = Vp9DecAddBuffer(vpi_ctx->dec_inst, &mem);
                    VPILOGD("after Vp9DecAddBuffer ret = %d\n", ret);
...
int vpi_decode_vp9_dec_process(VpiDecCtx *vpi_ctx)
{
    ...
    vpi_decode_vp9_frame_decoding(vpi_ctx);
...
#ifdef USE_EXTERNAL_BUFFER
VpiRet vpi_dec_vp9_get_buffer_info(VpiDecInst inst,
...
#endif
    if (rv) {
        VPILOGD("Vp9DecGetBufferInfo ret %s\n", dec_ret_string(rv));
        return VPI_ERR_UNKNOWN;
    }
    return VPI_SUCCESS;
}

VpiRet vpi_dec_vp9_add_buffer(VpiDecInst inst, struct DWLLinearMem *buf)
{
    enum DecRet rv;

    rv = Vp9DecAddBuffer(inst, buf);
    if (rv) {
        VPILOGD("Vp9DecAddBuffer ret %s\n", dec_ret_string(rv));
        return VPI_ERR_UNKNOWN;
    }
    return VPI_SUCCESS;
}

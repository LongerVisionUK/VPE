...
static i32 copy_qp_delta_2_memory(VPEH26xEncOptions *options, VCEncInst enc,
...
static i32 clear_qp_delta_map(VPEH26xEncOptions *options, VCEncInst enc,
                              struct VPIH26xEncCfg *tb)
{
    i32 ret        = OK;
    ...
    if (((struct vcenc_instance *)enc)->asic.regs.asicCfg.roiMapVersion >= 1)
        block_size *= 2;
#ifdef USE_OLD_DRV
    memset(tb->roi_map_delta_qp_mem->virtualAddress, 0, block_size);
#else
    memset(tb->roi_map_delta_qp_mem->rc_virtualAddress, 0, block_size);
    if (EWLTransDataRC2EP(tb->ewl, tb->roi_map_delta_qp_mem,
...
static int copy_flags_map_2_memory(VPEH26xEncOptions *options, VCEncInst enc,
                                   struct VPIH26xEncCfg *vpi_h26xe_cfg)
{
    u32 ctb_per_row =
        ((options->width + options->max_cu_size - 1) / (options->max_cu_size));
...
u32 setup_input_buffer(struct VPIH26xEncCfg *vpi_h26xe_cfg,
                       VPEH26xEncOptions *options, VCEncIn *p_enc_in)
{
    u32 src_img_size;
...
static i32 setup_roi_map_ver3(struct VPIH26xEncCfg *tb,
                              VPEH26xEncOptions *options, VCEncInst encoder)
{
    u8 *memory;
    u8 u8RoiData[24];
    ...
    i32 ctb_block_num, mb_block_num, block_cu8_rnum, block_cu8_cnum,
        last_block_cu8_cnum, block_cu8_num, ctb_row, ctb_column, read_size;
    u64 block_base_addr;
...
void init_slice_ctl(struct VPIH26xEncCfg *tb, struct VPEH26xEncOptions *options)
{
    int i;

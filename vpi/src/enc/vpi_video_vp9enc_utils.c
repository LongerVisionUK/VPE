...
static void vp9_dec_frame_info_print(struct DecPicturePpu *picture)
{
    int i;

    VPILOGD("decode frame info is\n");
    for (i = 0; i < DEC_MAX_OUT_COUNT; i++) {
        VPILOGD("Decpic[%d].w = %d \n", i, picture->pictures[i].pic_width);
        VPILOGD("Decpic[%d].h = %d \n", i, picture->pictures[i].pic_height);
        VPILOGD("Decpic[%d].stride = %d \n", i,
                picture->pictures[i].pic_stride);
        VPILOGD("Decpic[%d].stride_ch = %d \n", i,
                picture->pictures[i].pic_stride_ch);
        VPILOGD("Decpic[%d].seq.stride = %d \n", i,
                picture->pictures[i].sequence_info.pic_stride);
        VPILOGD("Decpic[%d].seq.stride_ch = %d \n", i,
                picture->pictures[i].sequence_info.pic_stride_ch);
        VPILOGD("Decpic[%d].ppenablde = %d \n", i,
                picture->pictures[i].pp_enabled);
        VPILOGD("Decpic[%d].dec_compres = %d \n", i,
                picture->pictures[i].pic_compressed_status);
        VPILOGD("Decpic[%d].seq.depth_l = %d \n", i,
                picture->pictures[i].sequence_info.bit_depth_luma);
        VPILOGD("Decpic[%d].seq.depth_ch = %d \n", i,
                picture->pictures[i].sequence_info.bit_depth_chroma);
        VPILOGD("Decpic[%d].pic_info->fmt = %d \n", i,
                picture->pictures[i].picture_info.format);
        VPILOGD("Decpic[%d].pic_info->pixel_format = %d \n", i,
                picture->pictures[i].picture_info.pixel_format);
        VPILOGD("Decpic[%d].seq.is_interlaced = %d \n", i,
                picture->pictures[i].sequence_info.is_interlaced);
        VPILOGD("Decpic[%d].seq.is_mono = %d \n", i,
                picture->pictures[i].sequence_info.is_mono_chrome);
    }
}
...
void vp9enc_print_total(VpiEncVp9Ctx *ctx)
{
    ...
    if (ctx->frm_cnt_hw_total > 0) {
      ...
      VPILOGD("HW DRAM BW Write: Avg %-6u Max %-6u [bytes/MB]\n",
        ...
        VPILOGE("ecfg->frm_cnt_hw_total:[%d] invalid values\n",
                ctx->frm_cnt_hw_total);
    }
}
...
static int vp9_pre_encode(VpiEncVp9Ctx *ctx)
{
    ...
    int encodeFail = 0;
    int i = 0, next = 0, src_img_size;
    int pic_width             = ecfg->lum_width_src;
    int pic_height            = ecfg->lum_height_src;
    const int first_pass_only = (ecfg->passes == 2 && ecfg->pass == 1) ? 1 : 0;
...
static int vp9_allocate_resource(VpiEncVp9Ctx *ctx)
{
    ...
    int frames = ecfg->lag_in_frames || ecfg->ref_frame_scheme == 4 ?
...
static int vp9_set_rate_control(VpiEncVp9Ctx *ctx, VpiEncVp9Setting *ecfg)
{
    ...
    if (ecfg->intrs_pic_rate != 0)
        rc_config.intraPictureRate = ecfg->intrs_pic_rate;
...
int vp9enc_set_ppindex(VpiEncVp9Ctx *ctx, VpeFrame *frame)
{
    int ret      = 0;
    int pp_index = 0;

    for (int i = 0; i < 5; i++) {
        VpePicInfo *pic_info = &frame->pic_info[i];
        VpePicData *pic_data = &pic_info->picdata;

        VPILOGD("Pic[%d][enable=%d, format=%d, width=%d, height=%d\n", i,
                pic_info->enabled, pic_info->format, pic_info->width,
                pic_info->height);

        VPILOGD("Pic[%d].crop[enabled=%d,x=%d,y=%d,w=%d,h=%d\n", i,
                pic_info->crop.enabled, pic_info->crop.x, pic_info->crop.y,
                pic_info->crop.w, pic_info->crop.h);

        VPILOGD("Pic[%d].scale[enable=%d,w=%d,h=%d\n", i,
                pic_info->scale.enabled, pic_info->scale.w, pic_info->scale.h);

        VPILOGD("Pic.Data[%d].is_interlaced = %d.\n", pic_data->is_interlaced);
        VPILOGD("\tpic_format = %d.\n", pic_data->pic_format);
        VPILOGD("\tpic_pixformat = %d.\n", pic_data->pic_pixformat);
        VPILOGD("\tbit_depth_luma = %d.\n", pic_data->bit_depth_luma);
        VPILOGD("\tbit_depth_chroma = %d.\n", pic_data->bit_depth_chroma);
        VPILOGD("\tPpic_compressed_status = %d.\n",
                pic_data->pic_compressed_status);
    }

    for (pp_index = 0; pp_index < 5; pp_index++) {
        if (frame->pic_info[pp_index].enabled == 1) {
            break;
        }
    }

    if (pp_index == 5) {
        VPILOGE("can't find pp_index\n");
        return -1;
    }
 ...
 */
int vp9enc_updatesetting_fromframe(VpiEncVp9Ctx *ctx, VpeFrame *in,
                                   VpiEncVp9Setting *out)
{
    VpiEncVp9Setting *ecfg = out;
    VpeFrame *frame        = in;
    u32 width, height;
    VpePicInfo *pic_info = &frame->pic_info[ctx->pp_index];
    VpePicData *pic_data = &pic_info->picdata;

 ...
 */
static int h26x_enc_open_encoder(VPEH26xEncOptions *options, VCEncInst *p_enc,
...
static int h26x_enc_call_vcstart(struct VpiH26xEncCtx *enc_ctx,
                                 VCEncOut *enc_out)
{
    VPEH26xEncOptions *options          = &enc_ctx->options;
    i32 ret                             = OK;
...
static int h26x_enc_call_vcencoding(struct VpiH26xEncCtx *enc_ctx,
                                    VpeH26xEncInAddr *p_addrs, void *indata,
                                    void *outdata)
{
    VPEH26xEncOptions *options = &enc_ctx->options;
    struct VPIH26xEncCfg *vpi_h26xe_cfg =
    ...
    int *stream_size       = &enc_ctx->stream_size;
    VpeH26xEncInAddr addrs = *p_addrs;
...
static int h26x_enc_encode(struct VpiH26xEncCtx *enc_ctx,
                           VpeH26xEncInAddr *p_addrs, VCEncOut *enc_out)
{
    VPEH26xEncOptions *options = &enc_ctx->options;
    i32 ret                    = OK;
    struct VPIH26xEncCfg *vpi_h26xe_cfg =
...
int vpi_h26xe_init(struct VpiH26xEncCtx *enc_ctx, H26xEncCfg *enc_cfg)
{
    ...
    enc_ctx->pp_index         = enc_cfg->pp_index;
...
int vpi_h26xe_encode(struct VpiH26xEncCtx *enc_ctx, void *input, void *output)
{
    ...
    VpeH26xEncInAddr *p_addrs     = NULL;
    VPEH26xEncOptions *options    = &enc_ctx->options;
    VpeH26xEncInAddr vpe_h26x_enc_in_addr;

    VpeFrame *vpe_frame = (VpeFrame *)input;
    p_addrs             = (VpeH26xEncInAddr *)vpe_frame->data[0];
    if (enc_ctx->find_pict == 1) {
        enc_ctx->find_pict = 0;

        pic_ppu  = (struct DecPicturePpu *)vpe_frame->data[0];
        ...
        if (options->enc_index == 0 && options->lookahead_depth &&
            ...
            if (!pic_data->pp_enabled) {
                printf("%s,%d when need 1/4 1pass input, pp1 should be enabled! !!vpe_frame->data[0] = (uint8_t *)&ctx->addrs!! \n",
                       __FUNCTION__, __LINE__);
                /*vpe_frame->data[0] = (uint8_t *)&ctx->addrs;*/
                return -1;
            }
        ...
        p_addrs = &vpe_h26x_enc_in_addr;
        memset(p_addrs, 0, sizeof(vpe_h26x_enc_in_addr));
    }

    VCEncOut *enc_out     = (VCEncOut *)&enc_ctx->enc_out;
    VCEncIn *p_enc_in     = (VCEncIn *)&(enc_ctx->vpi_h26xe_cfg.enc_in);
    VpePacket *vpe_packet = (VpePacket *)output;
    ...
    if (enc_ctx->no_input_pict == 1) {
        ...
        case VPIH26X_FLUSH_IDLE:
            flushRet = h26x_enc_encode(enc_ctx, p_addrs, enc_out);
            ...
            if (flushRet != VCENC_ERROR) {
                if (flushRet == VCENC_FRAME_READY) {
        ...
        case VPIH26X_FLUSH_ENCDATA:
            flushRet = h26x_enc_flush(enc_ctx, enc_out);
            if (flushRet == VCENC_FRAME_READY) {
                ret = VPI_ENC_FLUSH_ENCDATA_READY;
            } else if (flushRet == VCENC_FRAME_ENQUEUE) {
                ret = VPI_ENC_FLUSH_ENCDATA_FRM_ENQUEUE;
            } else if (flushRet == VCENC_OK) {
        ...
        case VPIH26X_FLUSH_FINISH:
            if (enc_ctx->encoder_is_end != HANTRO_TRUE) {
                flushRet = h26x_enc_end(enc_ctx, enc_out);
                VPILOGE("+++ h26x_enc_end ret = %d, FLUSH_FINISH \n", flushRet);
                if (flushRet == VCENC_OK) {
        ...
        vpe_packet->index_encoded = enc_out->indexEncoded;
        if (enc_ctx->trans_flush_pic != HANTRO_TRUE)
            vpe_packet->size = enc_ctx->stream_size;
        else
            vpe_packet->size = enc_out->streamSize;
        ...
        vpe_packet->data =
            (uint8_t *)enc_ctx->vpi_h26xe_cfg.outbuf_mem_factory[0][0]
                .rc_virtualAddress;
#endif
        vpe_packet->pts     = enc_out->pts;
        vpe_packet->pkt_dts = enc_ctx->vpi_h26xe_cfg.picture_enc_cnt;

        return ret;

    } else {
        p_enc_in->pts = vpe_frame->pts;
 ...
 *
 *  @Params: enc_ctx The context of Vpi H26x encoder
 *  @Params: vpi_ctrl_type The control command
 *  @Params: vpi_value The parameter for the command
 *  @Return: 0 for success, -1 for error
 */
...
{
    i32 ret                  = OK;
    VpiH26xEncCtrl *vpi_ctrl = (VpiH26xEncCtrl *)vpi_ctrl_type;
    struct VPIH26xEncCfg *vpi_h26xe_cfg =
        (struct VPIH26xEncCfg *)&enc_ctx->vpi_h26xe_cfg;
    VCEncIn *p_enc_in = (VCEncIn *)&(vpi_h26xe_cfg->enc_in);

    switch (*vpi_ctrl) {
    case VPI_H26x_ENC_CTRL_GET_NEXT_PIC:
        *(int *)vpi_value =
            h26x_enc_next_picture(vpi_h26xe_cfg, vpi_h26xe_cfg->picture_cnt) +
            vpi_h26xe_cfg->first_pic;
        p_enc_in->indexTobeEncode = *(int *)vpi_value;
        break;
    case VPI_H26x_ENC_CTRL_SET_FINDPIC_PPINDEX:
        enc_ctx->find_pict = 1;
        enc_ctx->pp_index  = *(int *)vpi_value;
        break;
    case VPI_H26x_ENC_CTRL_GET_FLUSHSTATE:
        *(int *)vpi_value = enc_ctx->flush_state;
        break;
    case VPI_H26x_ENC_CTRL_UPDATE_STATISTIC:
        h26x_enc_update_statistic(enc_ctx, (int *)vpi_value);
        break;
    case VPI_H26x_ENC_CTRL_NO_INFRM:
        enc_ctx->no_input_pict = *(int *)vpi_value;
        break;
    default:
        break;
    }

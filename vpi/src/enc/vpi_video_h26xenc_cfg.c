...
{
    VPEH26xEncOptions *options = &vpi_h26xe_ctx->options;
...
static int h26x_enc_profile_check(enum VpiH26xCodecID codec, const char *codec_name,
                                  char *profile, VPEH26xEncOptions *options)
{
...
static int h26x_enc_level_check(enum VpiH26xCodecID codec, const char *codec_name,
                                char *level, VPEH26xEncOptions *options)
{
    if (codec == CODEC_ID_HEVC) {
        if (strcmp(level, "1") == 0) {
                                 ...
                                 H26xEncCfg *h26x_enc_cfg)
{
    ...
    struct VPIH26xEncCfg *vpi_h26xe_cfg =
        (struct VPIH26xEncCfg *)&vpi_h26xe_ctx->vpi_h26xe_cfg;

    VPILOGE("h26x_enc_cfg->pp_index = %d\n", h26x_enc_cfg->pp_index);
    VPILOGE("pic %dx%d\n",
            h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index].width,
            h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index].height);

    if (h26x_enc_get_profile_and_level(vpi_h26xe_ctx, h26x_enc_cfg) != 0) {
        return -1;
    }

    if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
            .picdata.is_interlaced == 0) {
        if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index].width <
            h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index].pic_width) {
            options->lum_width_src =
                h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
                    .pic_width;
        } else {
            options->lum_width_src =
                NEXT_MULTIPLE(h26x_enc_cfg->frame_ctx
                                  ->pic_info[h26x_enc_cfg->pp_index]
                                  .width,
                              4);
        }
    } else {
    ...
    if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index].height <
        h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index].pic_height) {
    ...
    if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
            .picdata.is_interlaced == 0) {
        if ((h26x_enc_cfg->pp_index == VPI_DEC_OUT_RFC) ||
            (h26x_enc_cfg->pp_index == VPI_DEC_OUT_PP0)) {
            if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
    ...
    switch (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
                .picdata.pic_format) {
    ...
    case DEC_OUT_PIXEL_DEFAULT: /*reference frame with no compress and pp out 8bit is this pixel format*/
        ...
        if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
            ...
            } else if (h26x_enc_cfg->frame_ctx->pic_info[h26x_enc_cfg->pp_index]
    ...
    if (options->enc_index == 0 && options->lookahead_depth) {
        if (h26x_enc_cfg->pp_index != 0 && h26x_enc_cfg->pp_index != 1) {
...
static int h26x_enc_preset_check(char *input_param, VpeH26xPreset *preset)
{
                         ...
                         H26xEncCfg *h26x_enc_cfg)
{
    ...
    if ((h26x_enc_cfg->pp_index == VPI_DEC_OUT_RFC) ||
        ...
        options->enc_index = h26x_enc_cfg->pp_index - 1;
    }
    ...
    vpi_h26xe_ctx->h26x_enc_param_table =
        (VPEH26xParamsDef *)&h26x_enc_param_table[0];
        ...
        while (1) {
            ...
            if (strlen(param_name) != 0 && strlen(param_val) != 0) {
                ...
                if (h26x_enc_get_params_from_cmd(vpi_h26xe_ctx, param_name,
                                                 param_val) < 0) {
                    VPILOGE("Error parsing option '%s=%s'. Please check if it is valid hantro-params option.\n",
    ...
    VPILOGE("after get hantro-params from cmd !\n");
    h26x_enc_params_value_print(vpi_h26xe_ctx);
...
int h26x_enc_check_area(VCEncPictureArea *area, VPEH26xEncOptions *options)
{
            ...
            if (vpi_h26xe_cfg->output_rate_denom) {
                enc_statistic.bitrate_avg =
                    ...
                    (enc_statistic.frame_count *
                     vpi_h26xe_cfg->output_rate_denom);
            }
 ...
 */
int h26x_enc_alloc_res(VPEH26xEncOptions *cmdl, VCEncInst enc,
                       struct VPIH26xEncCfg *vpi_h26xe_cfg)
{
    i32 ret;
    u32 picture_size    = 0;
...
int h26x_enc_init_input_line_buffer(inputLineBufferCfg *line_buf_cfg,
                                    VPEH26xEncOptions *options, VCEncIn *encIn,
                                    VCEncInst inst,
                                    struct VPIH26xEncCfg *vpi_h26xe_cfg)
{

...
VpiEncSetting vp9enc_options[] = {
    { "intrs_pic_rate", OFFSET(intrs_pic_rate), 0, 300 },
    { "bitrate_window", OFFSET(bitrate_window), 0, 255 },
    { "qp_hdr", OFFSET(qp_hdr), 0, 255 },
    { "qp_min", OFFSET(qp_min), 0, 255 },
    { "qp_max", OFFSET(qp_max), 0, 255 },
    { "fixed_intra_qp", OFFSET(fixed_intra_qp), 0, 1 },
    { "picRc", OFFSET(pic_rc), 0, 4 },
    { "mcomp_filter_type", OFFSET(mcomp_filter_type), 0, 1 },
    /*   { "force8bit",OFFSET(force_8bit), 0, 5}, */
    { "effort", OFFSET(effort), 0, 5 },
    { "ref_frame_scheme", OFFSET(ref_frame_scheme), 0, 6 },
    { "filte_level", OFFSET(filter_level), 0, 4 },
    { "filter_sharpness", OFFSET(filter_sharpness), 0, 8 },
    { "lag_in_frames", OFFSET(lag_in_frames), 0, 255 },
    { "passes", OFFSET(passes), 0, 2 },
};

static int vpi_venc_vp9_convert_setting(VpeEncVp9Opition *in,
                                        VpiEncVp9Setting *out)
{
    VpeFrame *frame = (VpeFrame *)in->framectx;
...
{
    VpeEncVp9Opition *vpe_setting = (VpeEncVp9Opition *)cfg;
    VpiEncVp9Setting *cml         = &ctx->vp9_enc_cfg;
    VpeFrame *frame               = (VpeFrame *)vpe_setting->framectx;
    int ret                       = 0;
    ...
    ret = vp9enc_set_ppindex(ctx, frame);
    if (ret != 0) {
        VPILOGE("VP9EncDefaultParam error\n");
        goto error;
    }
    ...
    ret = vpi_venc_vp9_convert_setting(vpe_setting, cml);
    if (ret != 0) {
    ...
    if (vpe_setting->enc_params) {
        ret = vpi_enc_parse_param(vpe_setting->enc_params, vp9enc_options,
...
{
    VpeFrame *input             = (VpeFrame *)in;
    VpePacket *output           = out;
...
begin:
    ...
    /*Get back wait_for_enc_cnt from ffmpeg*/
    wait_for_enc_cnt = ctx->input_frame_wait_cnt;

    pic = (struct DecPicturePpu *)input->data[0];
    if (pic) {
        enc_data = &pic->pictures[ctx->pp_index];
        VPILOGD("Dump pic: %p %p %p %p\n", enc_data->luma.bus_address,
    ...
    case VP9ENC_FRAME_READY:
        ...
        if ((ctx->enc_out.frameSize == 0) && (ctx->enc_out.show_frame == 0))
        /*&& (cfg->ff_draining == 1)) */ {
            /* in flush status,encode one more time; */
            VPILOGD("encode one more time!\n");
            input = NULL;
            goto begin;
        }
        break;
    ...
    if (ret == VP9ENC_FIRSTPASS_FAILED) {
    ...
    VPILOGD("VP9DBG lag_in_frames=%d,lag_status =%d, show_frame=%d,pic_tobe_free=%d\n",
...
err:
    /*if encode failed then return faile value */
    ctx->pic_tobe_free = 0;
    return encodeFail != 0 ? -1 : 0;
}
...
int vpi_venc_vp9_control(VpiEncVp9Ctx *ctx, void *indata, void *outdata)
{
    ...
    } else if (cmd->cmd == VPE_CMD_VP9ENC_GET_PIC_TOBE_FREE) {

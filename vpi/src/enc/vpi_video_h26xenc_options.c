...
#define OFFSETM_VCE(x) offsetof(VPEH26xEncOptions, x)
VPEH26xParamsDef h26x_enc_param_table[] = {
    { "force8bit",
      'U',
      TYPE_INT,
      0,
    ...
    { "cpb_size",
      ...
      { .i64 = 1000000 },
      OPT_FLAG_VCE | OPT_FLAG_MULTI,
      "[VC8000E] HRD Coded Picture Buffer size in bits. (default [1000000], suggest 2*bitrate)." },
    { "gop_size",
      COM_SHORT,
      TYPE_INT,
      0,
...
static int parse_type_arg(VPEH26xParamsDef *p_param_line, void *p)
{
...
{
    VPEH26xParamsDef *p_param_line;
    VPEH26xEncOptions *options = (VPEH26xEncOptions *)&vpi_h26xe_ctx->options;
    int i;
    u8 *p = NULL;
...
static const VPEH26xParamsDef *find_option(const VPEH26xParamsDef *po,
                                           const char *name)
{
    int len  = strlen(name);
    int flag = 0;
...
int h26x_enc_get_params_from_cmd(struct VpiH26xEncCtx *vpi_h26xe_ctx,
                                 const char *name, const char *input_value)
{
    VPEH26xEncOptions *options = (VPEH26xEncOptions *)&vpi_h26xe_ctx->options;
    const VPEH26xParamsDef *p_param_line = NULL;
    u8 *p                                = NULL;
    ...
    if ((enum AVOptionType)p_param_line->type == AV_OPT_TYPE_INT) {
        int64_t value = atoi(input_value);
        if (p_param_line->min != NOCARE) {
            if (value < p_param_line->min) {
                if (value != 0 && value != DEFAULT_VALUE) {
                    printf("option %s value %ld less than min %d or not 0 nor DEFAULT_VALUE!\n",
        ...
        if (p_param_line->max != NOCARE) {
            if (value > p_param_line->max) {
                printf("option %s value %ld greater than max %d!\n",
        ...
        *((int *)p) = value;
        printf("%s set to %d\n", p_param_line->name, *((int *)p));
    } else if ((enum AVOptionType)p_param_line->type == AV_OPT_TYPE_FLOAT) {
        *((float *)p) = atof(input_value);
    } else if ((enum AVOptionType)p_param_line->type == AV_OPT_TYPE_STRING) {

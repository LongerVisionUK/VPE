...
long trans_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if ((cid >= IOCTL_CMD_PCIE_MINNR) && (cid <= IOCTL_CMD_PCIE_MAXNR))
		...
		if (mutex_lock_interruptible(&tdev->tcache[val].lock))
			return -ERESTARTSYS;
		...
		mutex_unlock(&tdev->tcache[val].lock);
...
static int trans_close(struct inode *inode, struct file *filp)
{
	...
	for (i = 0; i < 2; i++) {
		if (tdev->tcache[i].filp == filp) {
			mutex_unlock(&tdev->tcache[i].lock);
			   ...
			   const struct pci_device_id *id)
{
	...
	mutex_init(&tdev->tcache[0].lock);
	mutex_init(&tdev->tcache[1].lock);
	tdev->hw_err_flag = 0;

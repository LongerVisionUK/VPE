...
#define S0_BLOCK_CNT	16
#define S1_BLOCK_CNT	16
	...
	*addr = 0;
	...
	for (i = 0; i < blk_cnt; i++) {
		if (mem_block[i].task_id == 0) {
			...
			if (ret) {
				trans_dbg(tmem->tdev, TR_ERR,
					"mem: the size:0x%x is very large, more than block:%d size\n",
					size, i);
			} else {
				...
				struct device_attribute *attr, char *buf)
{
	...
	pos += sprintf(buf + pos,
		"   S0: %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d\n",
		s0_bk[0].task_id, s0_bk[1].task_id, s0_bk[2].task_id,
		s0_bk[3].task_id, s0_bk[4].task_id, s0_bk[5].task_id,
		s0_bk[6].task_id, s0_bk[7].task_id, s0_bk[8].task_id,
		s0_bk[9].task_id, s0_bk[10].task_id, s0_bk[11].task_id,
		s0_bk[12].task_id, s0_bk[13].task_id, s0_bk[14].task_id,
		s0_bk[15].task_id);
	pos += sprintf(buf + pos,
		"   S1: %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d  %2d\n",
		s1_bk[0].task_id, s1_bk[1].task_id, s1_bk[2].task_id,
		s1_bk[3].task_id, s1_bk[4].task_id, s1_bk[5].task_id,
		s1_bk[6].task_id, s1_bk[7].task_id, s1_bk[8].task_id,
		s1_bk[9].task_id, s1_bk[10].task_id, s1_bk[11].task_id,
		s1_bk[12].task_id, s1_bk[13].task_id, s1_bk[14].task_id,
		s1_bk[15].task_id);
...
int cb_mem_init(struct cb_tranx_t *tdev)
{
	...
	trans_dbg(tdev, TR_DBG,
		...
		(s0_end - s0_start) / S0_BLOCK_CNT,
		(s0_end - s0_start) / S0_BLOCK_CNT / CHUNK_SIZE);

	trans_dbg(tdev, TR_DBG,
		"memory : s1_start:0x%08lx s1_size:0x%lx block_cnt:%d, block_size:0x%lx,chunks:%ld.\n",
		s1_start, s1_end - s1_start, S1_BLOCK_CNT,
		(s1_end - s1_start) / S1_BLOCK_CNT,
		(s1_end - s1_start) / S1_BLOCK_CNT / CHUNK_SIZE);

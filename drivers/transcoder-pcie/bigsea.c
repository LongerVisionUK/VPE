...
static const
unsigned int bigsea_cores[BIGSEA_MAX_CORES][3] = {
...
static int bigsea_check_irq(struct bigsea_t *tbigsea, int id)
{
	...
	if (tbigsea->core[id].irq_rcvd == 1) {
		spin_lock_irqsave(&tbigsea->irq_lock, flags);
		tbigsea->core[id].irq_rcvd = 0;
		spin_unlock_irqrestore(&tbigsea->irq_lock, flags);
		rdy = 1;
	}
...
static int bigsea_reserve_core(struct cb_tranx_t *tdev,
...
static int bigsea_release_core(struct cb_tranx_t *tdev, u32 id,
				     struct file *filp)
{
	u32 st;
	struct bigsea_t *tbigsea = tdev->modules[TR_MODULE_BIGSEA];

	st = readl(tbigsea->core[id].hwregs + BIGSEA_IRQ_STAT_OFF);
	/* make sure HW is disabled */
	if (st & BIGSEA_ENABLE) {
		trans_dbg(tdev, TR_ERR,
			"bigsea: core:%d status is enabled, force reset\n", id);

		/* abort codec */
		st |= BIGSEA_ABORT | BIGSEA_IRQ_DISABLE;
		writel(st, tbigsea->core[id].hwregs+BIGSEA_IRQ_STAT_OFF);
	}
	...
	for (id = 0; id < BIGSEA_MAX_CORES; id++) {
		if (tbigsea->core[id].filp == filp) {
			trans_dbg(tdev, TR_DBG,
				  "bigsea: %s core:%d\n", __func__, id);
			bigsea_release_core(tdev, id, filp);
		}
	}
}

/* wait core irq, BIGSEA_WAIT_TIMEOUT is timeout time. */
static int bigsea_wait_ready(struct bigsea_t *tbigsea,
				   ...
				   struct file *filp)
{
	...
	trans_dbg(tbigsea->tdev, TR_DBG, "bigsea: %s core:%d\n", __func__, id);

	ret = wait_event_interruptible_timeout(tbigsea->codec_wait_queue,
			bigsea_check_irq(tbigsea, id), BIGSEA_WAIT_TIMEOUT);
	if (ret == 0) {
		trans_dbg(tbigsea->tdev, TR_ERR,
...
irqreturn_t bigsea_isr(int index, void *data)
{
	...
	if (spin_trylock_irqsave(&tbigsea->irq_lock, flags) == 0)
		return IRQ_NONE;
	...
	if (handled)
		wake_up_interruptible_all(&tbigsea->codec_wait_queue);
...
int bigsea_init(struct cb_tranx_t *tdev)
{
	...
	if (!tbigsea) {
		trans_dbg(tdev, TR_ERR,
			"bigsea: %s kmalloc bigsea dev failed\n", __func__);
		goto out;
	}
		      ...
		      struct cb_tranx_t *tdev)
{
	...
	case CB_TRANX_BIGSEA_RESERVE:
		if (copy_from_user(&info, argp, sizeof(struct core_info))) {
			trans_dbg(tdev, TR_ERR,
				"bigsea: reserve copy from user failed\n");
			return -EFAULT;
		}
	...
	case CB_TRANX_BIGSEA_RELEASE:
		...
		if (id >= tbigsea->cores) {
			trans_dbg(tdev, TR_ERR,
				"bigsea: release core id:%d error.\n", id);
			return -EFAULT;
		}
		ret = bigsea_release_core(tdev, id, filp);
		break;
	case CB_TRANX_BIGSEA_WAIT_DONE:
		if (copy_from_user(&core, argp, sizeof(struct core_desc))) {
			trans_dbg(tdev, TR_ERR,
				"bigsea: wait copy from user failed\n");
			return -EFAULT;
		}

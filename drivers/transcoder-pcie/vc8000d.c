...
#define VCD_APP_TIMEOUT		2000000

#define VCD_CLOCK_CONTROL
#define VCD_DGB

#ifndef EMULATOR
#define VC8000D_TIMEOUT		(2*HZ) /* second */
#else
...
static int vcd_a_sft_reset(struct cb_tranx_t *tdev, int core)
{
	...
	trans_dbg(tdev, TR_ERR, "vc8000d: %s core:%d %s.\n",
		  __func__, core, ((val&0x3) == 0x1) ? "done" : "failed");
...
static int vcd_b_sft_reset(struct cb_tranx_t *tdev, int core)
{
	...
	trans_dbg(tdev, TR_ERR, "vc8000d: %s core:%d %s.\n",
		  __func__, core, ((val&0xc) == 0x4) ? "done" : "failed");
...
int vc8000d_reset(struct cb_tranx_t *tdev)
{
	...
	vcd_enable_clock(tvcd, 0);
	vcd_enable_clock(tvcd, 1);
	vcd_enable_clock(tvcd, 2);
	vcd_enable_clock(tvcd, 3);
	...
	trans_dbg(tdev, TR_INF, "vc8000d: %s done.\n", __func__);
	return 0;
}
				       ...
				       char task_priority)
{
	...
	if (id != -1) {
		...
		id = new->core_id;
#ifdef VCD_DGB
		tvcd->core[id].bak_filp = new->filp;
#endif
		list_del(&new->rsv_list);
		new->used = 0;
		new->reserved = 0;
	...
	if (tvcd->core[id].core_status != IDLE_FLAG) {
		trans_dbg(tvcd->tdev, TR_ERR,
			"vc8000d: %s core_%d_status:0x%x error, line:%d filp=%p, rsv_cnt=%d en_cnt=%d irq_cnt=%d chk_irq_cnt=%d idle_cnt=%d\n",
			__func__, id, tvcd->core[id].core_status,
			__LINE__, filp, tvcd->core[id].rsv_cnt,
			tvcd->core[id].en_cnt, tvcd->core[id].irq_cnt,
			tvcd->core[id].chk_irq_cnt, tvcd->core[id].idle_cnt);
	}
	tvcd->core[id].core_status = RSV_FLAG;
	tvcd->core[id].rsv_cnt++;
	tvcd->core[id].rsv_s = ktime_get();
#endif
					...
					struct file *filp)
{
	...
	int c;
//	unsigned long flags;
	unsigned long rsv_en_us, en_irq_us, irq_chk_s, rel_irq_s;

	status = readl(tvcd->core[id].hwregs + VCD_IRQ_STAT_OFF);
#ifdef VCD_DGB
	tvcd->core[id].rel_s = ktime_get();
	en_irq_us =  ktime_to_us(ktime_sub(tvcd->core[id].irq_s,tvcd->core[id].enable_s));
	rel_irq_s =  ktime_to_us(ktime_sub(tvcd->core[id].rel_s,tvcd->core[id].irq_s));
	irq_chk_s =  ktime_to_us(ktime_sub(tvcd->core[id].check_irq_s, tvcd->core[id].irq_s));
	if ((filp != tvcd->core[id].filp)
		|| (tvcd->core[id].core_status != CHK_IRQ_FLAG)
		|| (en_irq_us >= VCD_APP_TIMEOUT)
		|| (irq_chk_s >= VCD_APP_TIMEOUT)
		|| (rel_irq_s >= VCD_APP_TIMEOUT)) {
		rsv_en_us =  ktime_to_us(ktime_sub(tvcd->core[id].enable_s,tvcd->core[id].rsv_s));
		trans_dbg(tvcd->tdev, TR_ERR,
			"vc8000d: %s filp=%p, core_filp=%p bak_filp=%p core_%d_status_0x%x, hw_status=0x%x \
rsv_en_us=%ld, en_irq_us=%ld, irq_chk_s=%ld, rel_irq_s=%ld \
rsv_cnt=%d en_cnt=%d irq_cnt=%d chk_irq_cnt=%d idle_cnt=%d\n",
			__func__, filp, tvcd->core[id].filp,
			tvcd->core[id].bak_filp, id, tvcd->core[id].core_status, status,
			rsv_en_us, en_irq_us, irq_chk_s, rel_irq_s,
			tvcd->core[id].rsv_cnt, tvcd->core[id].en_cnt,
			tvcd->core[id].irq_cnt, tvcd->core[id].chk_irq_cnt,
...
void vcd_close(struct cb_tranx_t *tdev, struct file *filp)
{
	...
	for (id = 0; id < VCD_MAX_CORES; id++) {
		if (tvcd->core[id].filp == filp) {
			trans_dbg(tvcd->tdev, TR_DBG,
				  "vc8000d: %s core:%d\n", __func__, id);
			vc8000d_release_core(tvcd, id, filp);
		}
	}
}

static int check_dec_irq(struct vc8000d_t *tvcd, const struct file *filp, u32 id)
{
	unsigned long flags;
	u32 status;
	unsigned long rsv_en_us, en_irq_us, irq_chk_s;
	int rdy = 0;

	if (tvcd->core[id].irq_rcvd)
	{
		spin_lock_irqsave(&tvcd->irq_lock, flags);
		tvcd->core[id].irq_rcvd = 0;
		rdy = 1;
		spin_unlock_irqrestore(&tvcd->irq_lock, flags);
#ifdef VCD_DGB
		tvcd->core[id].check_irq_s = ktime_get();
		en_irq_us =  ktime_to_us(ktime_sub(tvcd->core[id].irq_s,tvcd->core[id].enable_s));
		irq_chk_s =  ktime_to_us(ktime_sub(tvcd->core[id].check_irq_s, tvcd->core[id].irq_s));
		if ((tvcd->core[id].core_status != RCVD_IRQ_FLAG)
			...
			|| (irq_chk_s >= VCD_APP_TIMEOUT)) {
			rsv_en_us =  ktime_to_us(ktime_sub(tvcd->core[id].enable_s, tvcd->core[id].rsv_s));
			status = readl(tvcd->core[id].hwregs + 4);
			trans_dbg(tvcd->tdev, TR_ERR,
				"vc8000d: %s filp=%p, core_filp=%p bak_filp=%p core_%d_status_0x%x, hw_status=0x%x \
rsv_en_us=%ld, en_irq_us=%ld, irq_chk_s=%ld, \
rsv_cnt=%d en_cnt=%d irq_cnt=%d chk_irq_cnt=%d idle_cnt=%d\n",
			__func__, filp, tvcd->core[id].filp,
			tvcd->core[id].bak_filp, id, tvcd->core[id].core_status, status,
			rsv_en_us, en_irq_us, irq_chk_s,
			tvcd->core[id].rsv_cnt, tvcd->core[id].en_cnt,
			tvcd->core[id].irq_cnt, tvcd->core[id].chk_irq_cnt, tvcd->core[id].idle_cnt);
		}
...
long wait_dec_ready(struct vc8000d_t *tvcd, const struct file *filp, u32 id)
{
	int ret;
...
rsv_cnt=%d en_cnt=%d irq_cnt=%d chk_irq_cnt=%d idle_cnt=%d\n",
				...
				*core_id = id;
				rdy = 1;
				break;
			} else if (tvcd->core[id].filp == NULL) {
				/* zombie IRQ */
				/* read VCD_IRQ_STAT_OFF */
				tvcd->core[id].check_irq_s = ktime_get();
				status = readl(tvcd->core[id].hwregs + 4);
				trans_dbg(tvcd->tdev, TR_ERR,
					"vc8000d:  %s, irq on core:%d,but no owner, bak_filp=%p, core_status=0x%x hw_status=0x%x, filp=%p, time_us=%lld, tdev=%p, rsv_cnt=%d en_cnt=%d irq_cnt=%d chk_irq_cnt=%d idle_cnt=%d\n",
					__func__, id, tvcd->core[id].bak_filp,
					tvcd->core[id].core_status, status, filp,
					ktime_to_us(ktime_sub(tvcd->core[id].check_irq_s,tvcd->core[id].enable_s)),
					tvcd->tdev, tvcd->core[id].rsv_cnt,
					tvcd->core[id].en_cnt,  tvcd->core[id].irq_cnt,
					tvcd->core[id].chk_irq_cnt, tvcd->core[id].idle_cnt);
				tvcd->core[id].irq_rcvd = 0;
			}
...
irqreturn_t vcd_isr(int index, void *data)
{
	...
	unsigned long rsv_en_us, en_irq_us;

	if (spin_trylock_irqsave(&tvcd->irq_lock, flags) == 0)
		return IRQ_NONE;
	...
	if (EN_FLAG != core->core_status) {
		if (IDLE_FLAG != core->core_status) {
			irq_status_dec = readl(hwregs + VCD_IRQ_STAT_OFF);
			trans_dbg(tdev, TR_ERR, "vc8000d: %s, core_%d_status:0x%x err, hw_status:0x%x\n",
			__func__, index, core->core_status, irq_status_dec);
		} else
			trans_dbg(tdev, TR_ERR, "vc8000d: %s, core_%d_status:0x%x err\n",
	...
	if (tvcd->core[index].filp == NULL) {
		trans_dbg(tvcd->tdev, TR_ERR,
			"vc8000d: %s index:%d, owner is null, fatal error, hw_status=0x%x, tdev=%p.\n",
			__func__, index, irq_status_dec,tvcd->tdev);
	}

	if (irq_status_dec & ((1<<0)|(1<<2)|(1<<3)|(1<<5)))
		trans_dbg(tvcd->tdev, TR_ERR,
			"vc8000d: %s index:%d, hw_status=0x%x,tdev=%p\n",
			__func__, index, irq_status_dec,tvcd->tdev);

	if (irq_status_dec & VCD_DEC_IRQ) {
		core->irq_s = ktime_get();
		en_irq_us =  ktime_to_us(ktime_sub(tvcd->core[index].irq_s,tvcd->core[index].enable_s));
		if ((EN_FLAG != core->core_status)
			|| (en_irq_us > VCD_APP_TIMEOUT)) {
			rsv_en_us =  ktime_to_us(ktime_sub(tvcd->core[index].enable_s, tvcd->core[index].rsv_s));
			trans_dbg(tvcd->tdev, TR_ERR,
			"vc8000d: %s core_%d_status_0x%x hw_status=0x%x \
rsv_en_us=%ld, en_irq_us=%ld, \
rsv_cnt=%d en_cnt=%d irq_cnt=%d idle_cnt=%d\n",
			__func__, index, core->core_status, irq_status_dec,
			rsv_en_us, en_irq_us,
			core->rsv_cnt, core->en_cnt, core->irq_cnt, core->idle_cnt);
		}
	...
	if (handled) {
		wake_up_interruptible_all(&tvcd->dec_wait_queue);
	} else {

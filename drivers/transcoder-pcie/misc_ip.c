...
static inline void check_bar2_addr(struct cb_tranx_t *tdev,void __iomem *addr, int ip_id)
{
				 ...
				 struct cb_tranx_t *tdev, int ip_id)
{
	...
	if (ret)
		trans_dbg(tdev, TR_ERR, "misc_ip: %s failed.\n", __func__);
	        ...
	        if (ip_id == VC8000D_ID) {
	            if ((regs_info[regs_cnt-1].id == 1) && ((regs_info[regs_cnt-1].val&0x1) == 1)) {
	                video_core = &tvcd->core[core.id];
	                video_core->core_status = EN_FLAG;
	                video_core->en_cnt++;
	                video_core->enable_s = ktime_get();
	            }
	        }
				...
				struct cb_tranx_t *tdev, int ip_id)
{
	...
	for (i = 0; i < regs_cnt; i++) {
		...
		if ((ip_id == BIGSEA_ID) && (regs_info[i].id == BIGSEA_HW_STA))
			check_bigsea_hwerr(tdev, regs_info[BIGSEA_HW_STA].val);
	}

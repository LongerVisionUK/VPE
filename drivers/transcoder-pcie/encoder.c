 ...
 * them again.
 */
int enc_reset(struct cb_tranx_t *tdev)
{
	bigsea_enable_clock(tdev->modules[TR_MODULE_BIGSEA], 0);
	bigsea_enable_clock(tdev->modules[TR_MODULE_BIGSEA], 1);

	vce_enable_clock(tdev->modules[TR_MODULE_VC8000E], 1 << 0);
	vce_enable_clock(tdev->modules[TR_MODULE_VC8000E], 1 << 1);

	if (enc_sft_reset(tdev, 0))
		return -EFAULT;
	/*
	 ...
	 */
	ccm_write(tdev, 0x100B4, 0xFFFFFFFF);
	if (enc_sft_release(tdev, 0))
		return -EFAULT;

	if (enc_sft_reset(tdev, 1))
		return -EFAULT;
	ccm_write(tdev, 0x10124, 0xFFFFFFFF);
	if (enc_sft_release(tdev, 1))
		return -EFAULT;

	trans_dbg(tdev, TR_INF, "encoder: %s done.\n", __func__);
	return 0;
}
...
//	spin_lock_init(&tenc->loading_lock);

...
typedef enum { ENC_FLIP_NONE = 0, ENC_FLIP_VERT = 1 } EncFlipType;
...
typedef enum {
    ENC_RGBTOYUV_BT601_SS = 0, /* Color conversion according to BT.601, studio
                                  swing  */
    ENC_RGBTOYUV_BT709_SS = 1, /* Color conversion according to BT.709, studio
                                  swing  */
    ENC_RGBTOYUV_BT601_FS = 2, /* Color conversion according to BT.601, full
                                  swing    */
    ENC_RGBTOYUV_BT709_FS = 3, /* Color conversion according to BT.709, full
                                  swing    */
...
typedef struct {
    ...
    u32 origHeight; /* Input camera picture height. Tiled 8x4 and 4x4
                       multiple 8. */
    ...
    u32 cropWidth; /* Picture width after cropping. Will be scaled down to the
                      final encoded width. */
    u32 cropHeight; /* Picture height after cropping. Will be scaled down to the
    ...
    EncColorConversionType colorConversion; /* Define color conversion
                                               parameters for RGB input   */
    EncColorRangeConversion colorRangeConversion; /* Enable studio<->full color
                                                     range conversion */

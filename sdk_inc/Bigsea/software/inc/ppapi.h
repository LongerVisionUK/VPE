...
/* Picture bit-depth */
typedef enum { PPENC_8BIT = 0, PPENC_10BIT = 1 } PPEncBitDepth;
...
typedef enum { PPENC_QM_PSNR = 0, PPENC_QM_SSIM = 1 } PPEncQualityMetric;
...
PPEncRet PPEncInit(const PPEncConfig *pEncConfig, PPEncInst *instAddr);
...
PPEncRet PPEncGetHwPerfCounter(PPEncInst inst, HwPerfCounter *counters,
                               u32 *counter_count);
...
PPEncRet PPImagePreProcess(PPEncInst inst, PPEncOut *pEncOut,
                           SwRegisters *swregs);

...
enum DecErrorConcealment { DEC_PICTURE_FREEZE = 0, DEC_INTRA_FREEZE = 1 };
...
struct DecConfig {
    ...
    u32 disable_late_hw_sync; /**< For best performance, HW sync is done as late
                                 as possible, in practice when new packages is
                                 dispatched for decoding. Set this high if it is
                                 required that HW is idle when DecDecode()
                                 returns. */
    u32 use_reference_compression; /**< Use lossless compression for reference
                                      frames, if supported. */
    u32 mode;
};
...
struct DecPicture {
    ...
    unsigned char compression_info_tiles; /**< Compression control info number
                                             of tiles*/

...
#define PTRACE(fmt, ...)                                                       \
    FB_SYSLOG(ewl, SYSLOG_SINK_LEV_DEBUG_SW, "%s(%d): " fmt, __FUNCTION__,     \
              __LINE__, ##__VA_ARGS__)
#define ERROR_PRINT(fmt, ...)                                                  \
    FB_SYSLOG(ewl, SYSLOG_SINK_LEV_ERROR, "%s(%d): " fmt, __FUNCTION__,        \
              __LINE__, ##__VA_ARGS__)
#define REG_DEBUG(fmt, ...)                                                    \
    FB_SYSLOG(ewl, SYSLOG_SINK_LEV_DEBUG_M_IP, "%s(%d): " fmt, __FUNCTION__,   \
              __LINE__, ##__VA_ARGS__)
#else
...
#define PTRACE(...)                                                            \
    {                                                                          \
        if (fEwl == NULL) fEwl = fopen("ewl.trc", "w");                        \
        if (fEwl) {                                                            \
            fprintf(fEwl, "%s:%d:", __FILE__, __LINE__);                       \
            fprintf(fEwl, __VA_ARGS__);                                        \
        }                                                                      \
    }
#else
#define PTRACE(...)                                                            \

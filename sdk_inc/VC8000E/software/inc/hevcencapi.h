...
typedef enum {
    ...
    INPUT_FORMAT_YUV420_SEMIPLANAR_8BIT_COMPRESSED_FB = VCENC_FORMAT_MAX +
                                                        1, /*36*/
...
typedef enum {
    VCENC_RGBTOYUV_BT601 = 0, /* Color conversion of limited range[16,235]
                                 according to BT.601 */
    VCENC_RGBTOYUV_BT709 = 1, /* Color conversion of limited range[16,235]
                                 according to BT.709 */
    VCENC_RGBTOYUV_USER_DEFINED = 2, /* User defined color conversion */
    VCENC_RGBTOYUV_BT2020 = 3, /* Color conversion according to BT.2020 */
    VCENC_RGBTOYUV_BT601_FULL_RANGE = 4, /* Color conversion of full
                                            range[0,255] according to BT.601*/
    VCENC_RGBTOYUV_BT601_LIMITED_RANGE = 5, /* Color conversion of limited
                                               range[0,219] according to
                                               BT.601*/
    VCENC_RGBTOYUV_BT709_FULL_RANGE = 6 /* Color conversion of full range[0,255]
                                           according to BT.709*/
...
------------------------------------------------------------------------------*/
typedef struct {
    u32 *ctuOffset; /* point to a memory containing the total CU number by the
                       end of each CTU */
...
typedef struct {
    ...
    u8 interPredIdc; /* only for INTER cu. prediction direction. 0: by list0; 1:
                        by list1; 2: bi-direction */
    VCEncMv mv[2]; /* only for INTER cu. motion information. mv[0] for list0 if
                      it's valid; mv[1] for list1 if it's valid */
    u8 intraPartMode; /* only for INTRA cu. partition mode. 0:2Nx2N; 1: NxN */
    u8 intraPredMode[16]; /* only for INTRA CU. prediction mode. 0: planar; 1:
                             DC; 2-34: Angular in HEVC spec. intraPredMode[1~3]
                             only valid for NxN */
    u8 qp;
    u32 mean;
    u32 variance;
...
typedef struct {
    ...
    u32 refFrameAmount; /* Amount of reference frame buffers, [0..8]
                         ...
                         * 4 = gop size is 8
                         * 8 = gop size is 8 svct hirach 7B+1p 4 layer, only
                         * libva support this config*/
    ...
    u32 compressor; /*Enable/Disable Embedded Compression
                                        ...
                                        3 = Enable Both Luma and Chroma
                       Compression*/
    u32 interlacedFrame; /*0 = progressive frame; 1 = interlace frame  */

    u32 bitDepthLuma; /*luma sample bit depth of encoded bit stream, 8 = 8 bits,
                         9 = 9 bits, 10 = 10 bits  */
    u32 bitDepthChroma; /*chroma sample bit depth of encoded bit stream,  8 = 8
                           bits, 9 = 9 bits, 10 = 10 bits */
    ...
    u32 rdoLevel; /*control RDO hw runtime effort level, balence between quality
                   * and throughput performance, [0..2] 0 = RDO run 1x cadidates
    ...
    u32 P010RefEnable; /* enable P010 tile-raster format for reference frame
                          buffer */
    u32 ctbRcMode; /* CTB QP adjustment mode for Rate Control and Subjective
                      Quality. 0 = No CTB QP adjustment.\n" 1 = CTB QP
                      adjustment for Subjective Quality only." 2 = CTB QP
                      adjustment for Rate Control only." 3 = CTB QP adjustment
                      for both Subjective Quality and Rate Control. */
    ...
    u8 lookaheadDepth;
    u32 extDSRatio; /* external downsample ratio for first pass. 0=1/1 (no
                       downsample), 1=1/2 downsample */
...
typedef struct {
    i32 ref_pic; /*  delta_poc of this short reference picture relative to the
                    poc of current picture or index of LTR */
...
typedef struct {
    ...
    u32 numRefPics; /*  the number of reference pictures kept for this picture,
                       the value should be within [0, VCENC_MAX_REF_FRAMES] */
    VCEncGopPicRps refPics[VCENC_MAX_REF_FRAMES]; /*  short-term reference
                                                     picture sets for this
                                                     picture*/
} VCEncGopPicConfig;
...
typedef struct {
    ...
    VCEncGopPicRps refPics[VCENC_MAX_REF_FRAMES]; /*  short-term reference
                                                     picture sets for this
                                                     picture*/
    i32 i32Ltr; /* index of the long-term referencr frame,the value should be
                   within [0, VCENC_MAX_LT_REF_FRAMES]. 0 -- not
                   LTR; 1...VCENC_MAX_LT_REF_FRAMES--- LTR's index */
    i32 i32Offset; /* offset of the special pics, relative to start of
                      ltrInterval*/
    i32 i32Interval; /* interval between two pictures using LTR as referencr
                        picture or  interval between two pictures coded as
                        special frame */
    i32 i32short_change; /* only change short term coding parameter. 0 - not
                            change, 1- change */
} VCEncGopPicSpecialConfig;

typedef struct {
    VCEncGopPicConfig *pGopPicCfg; /* Pointer to an array containing all used
                                      VCEncGopPicConfig */
    u8 size; /* the number of VCEncGopPicConfig pointed by pGopPicCfg, the value
                should be within [0, MAX_GOP_PIC_CONFIG_NUM] */
    u8 id; /* the index of VCEncGopPicConfig in pGopPicCfg used by current
              picture, the value should be within [0, size-1] */
    u8 id_next; /* the index of VCEncGopPicConfig in pGopPicCfg used by next
                   picture, the value should be within [0, size-1] */
    u8 special_size; /* number of special configuration, the value should be
                        within [0, MAX_GOP_PIC_CONFIG_NUM] */
    i32 delta_poc_to_next; /* the difference between poc of next picture and
                              current picture */
    VCEncGopPicSpecialConfig *pGopPicSpecialCfg; /* Pointer to an array
                                                    containing all used
                                                    VCEncGopPicSpecialConfig */
    u8 ltrcnt; /* Number of long-term ref pics used,the value should be within
                  [0, VCENC_MAX_LT_REF_FRAMES]  */
    ...
    VCEncGopPicConfig *pGopPicCfgPass1; /* Pointer to an array containing all
                                           used VCEncGopPicConfig, used for
                                           pass1*/
    VCEncGopPicConfig *pGopPicCfgPass2; /* Pointer to an array containing all
                                           used VCEncGopPicConfig, used for
                                           pass2*/
} VCEncGopConfig;
...
typedef struct {
    ...
    u32 deblockOverride; /* flag to indicate whether deblock override between
                            slice */
    ...
    u32 sampleAspectRatioHeight; /* Vertical size of the sample aspect ratio
                                  ...
                                  */
    u32 enableCabac; /* [0,1] H.264 entropy coding mode, 0 for CAVLC, 1 for
                        CABAC */
    ...
    i32 pcm_loop_filter_disabled_flag; /* disable deblock filter for IPCM,
    ...
    u32 RoimapCuCtrl_index_enable; /* 0 = ROI map cu ctrl index disabled,  1 =
                                      ROI map cu ctrl index enabled */
    u32 roiMapDeltaQpBinEnable; /*0 = ROI map disabled, 1 = ROI map enabled.*/
    u32 RoimapCuCtrl_enable; /* 0 = ROI map cu ctrl disabled,  1 = ROI map cu
                                ctrl enabled */
    ...
    u32 noiseReductionEnable; /*0 = disable noise reduction; 1 = enable noise
                                 reduction */
    u32 noiseLow; /* valid value range :[1,30] , default: 10 */
    u32 firstFrameSigma; /* valid value range :[1,30] , default :11*/

    u32 gdrDuration; /*how many pictures it will take to do GDR, if 0, not do
                        GDR*/
    ...
    u32 streamMultiSegmentMode; /* 0:single segment 1:multi-segment with no sync
    ...
    /* back ground */
    i32 smartMeanTh[4];
    /* foreground/background threashold: maximum foreground pixels in background
     * block */
...
typedef struct {
    ...
    i32 smoothPsnrInGOP; /* dynamic bits allocation among GOP using SSE feed
                            back for smooth psnr, 1 - enable, 0 - disable */
    ...
    i32 ctbRcRowQpStep; /* max accumulated QP adjustment step per CTB Row by Ctb
                           Rate Control. QP adjustment step per CTB is
                           ctbRcRowQpStep/ctb_per_row. */
} VCEncRateCtrl;
...
typedef struct {
    ...
    ptr_t busChromaV; /* Bus address for input chrominance
                       ...
                       * interleaved format: not used
                       */
    ptr_t busLumaOrig; /* Bus address for original input picture when first-pass
                        * input is down-scaled planar format: luminance
                        * component semiplanar format: luminance component
                        * interleaved format: whole picture
                        */
    ptr_t busChromaUOrig; /* Bus address for original input chrominance when
                           * first-pass input is down-scaled planar format: cb
                           * component semiplanar format: both chrominance
                           * interleaved format: not used
                           */
    ptr_t busChromaVOrig; /* Bus address for original input chrominance when
                           * first-pass input is down-scaled planar format: cr
                           * component semiplanar format: not used interleaved
                           * format: not used
                           */
    ...
    i32 gopPicIdx; /* encoded order count of current picture within its GOP,
                      shoule be in the range of [0, gopSize-1] */
    ...
    u32 bIsIDR; /* coded current picture as IDR frame ,: false - not coded as
                   IDR, true-coded as IDR */
    u32 bIsPeriodUsingLTR; /* use LTR as referencr picture periodly: false - not
                              periodly used, true - periodly used */
    u32 bIsPeriodUpdateLTR; /* update LTR periodly:false - not periodly update,
                               true - periodly update */
    VCEncGopPicConfig gopCurrPicConfig; /* current pic running configuration */
    i32 long_term_ref_pic[VCENC_MAX_LT_REF_FRAMES]; /* LTR's poc:-1 - not a
                                                       valid poc, >= long-term
                                                       referencr's poc */
    u32 bLTR_used_by_cur[VCENC_MAX_LT_REF_FRAMES]; /* LTR used by current
                                                      picture: false- not used
                                                      as reference, true - used
                                                      as reference by current
                                                      picture */
    u32 bLTR_need_update[VCENC_MAX_LT_REF_FRAMES]; /* LTR's poc should update:
                                                      false - not need updated,
                                                      true- need updated */
    i8 i8SpecialRpsIdx; /* special RPS index selected by current coded picture,
                           -1 - NOT, >= 0 - special RPS */
    i8 i8SpecialRpsIdx_next; /* special RPS index selected by nextbeing coded
                                picture, -1 - NOT, >= 0 - special RPS,for h264
                              */
    u8 u8IdxEncodedAsLTR; /* current picture coded as LTR : 0 - not a LTR, 1...
                             VCENC_MAX_LT_REF_FRAMES -- idx of LTR */
    ...
    u32 bSkipFrame; /* code all MB/CTB in current frame as SKIP Mode. 0=No,
                       1=Yes */
...
typedef struct {
    ...
    u8 boolCurIsLongTermRef; /*Flag for current frame will be used as
                                LongTermReference for future frame*/

    double ssim[3]; /* Y/Cb/Cr SSIM values calculated by HW. Valid only if SSIM
                       calculation feature is supported by HW. */

    double psnr_y; /* PSNR_Y value, HW calcuate SSE, SW calculate log10f*/

    double psnr_y_predeb; /* PSNR_Y value (before deblock), HW calcuate SSE, SW
                             calculate log10f*/
    double psnr_cb_predeb; /* PSNR_CB value (before deblock), HW calcuate SSE,
                              SW calculate log10f*/
    double psnr_cr_predeb; /* PSNR_CR value (before deblock), HW calcuate SSE,
                              SW calculate log10f*/
...
typedef struct {
    ...
    // for 1/4 resolution first pass, for disable rfc datapath, xOffset and
    // yOffset should be 0, so not need ds params
    u32 origWidth_ds;
    u32 origHeight_ds;
    ...
    u32 inLoopDSRatio;
    u32 *virtualAddressScaledBuff; /*virtual address of  allocated buffer in
                                      aplication for scaled picture.*/
    ptr_t busAddressScaledBuff; /*phyical address of  allocated buffer in
                                   aplication for scaled picture.*/
    u32 sizeScaledBuff; /*size of allocated buffer in aplication for scaled
                           picture. the size is not less than
                           scaledWidth*scaledOutput*2 bytes */
...
typedef struct {
    ...
    u32 nalUnitInfoNum; /* Indicates how many information nal units are
                           completed, including all kinds of sei information.*/
    u32 nalUnitInfoNumPrev; /* Indicates how many information nal units are
                               completed at previous callback, including all
                               kinds of sei information.*/
...
u32 VCEncGetAlignedStride(int width, i32 input_format, u32 *luma_stride,
                          u32 *chroma_stride, u32 input_alignment);

/* Initialization & release */
VCEncRet VCEncInit(const VCEncConfig *config, VCEncInst *instAddr,
                   const void *ewl, const void *twoPassEwl);
 ...
 * When MVC mode is selected a prefix NAL unit is generated before view=0
 * frames.
 */
...
#if defined(SUPPORT_DEC400) || defined(SUPPORT_TCACHE)
i32 SetupPass2InputHwTransformer(VCEncInst inst, Pass2HWParam *privPass2HwParam,
                                 VCEncIn *encIn, const void *ewl);
i32 ReleasePass2InputHwTransformer(VCEncInst inst,
                                   Pass2HWParam *privPass2HwParam);
#endif
...
VCEncRet VCEncFlush(VCEncInst inst, const VCEncIn *pEncIn, VCEncOut *pEncOut,
                    VCEncSliceReadyCallBackFunc sliceReadyCbFunc);
...
VCEncPictureCodingType VCEncFindNextPic(VCEncInst inst, VCEncIn *encIn,

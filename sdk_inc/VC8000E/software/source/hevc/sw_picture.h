...
struct sw_picture {
    ...
    i32 frameNum; /* Frame number in decoding order, 0 for IDR, +1 for each
                     reference frame; used for H.264 */
    u32 nalRefIdc; /* current frame is used for reference */
    u32 markCurrentLongTerm; /* (H264 only) mark current frame as long term
                                reference */
    i32 curLongTermIdx; /* (H264 only) long term reference index of current
                           frame */
...
struct sw_picture *create_picture_ctrlsw(struct vcenc_instance *instance,
                                         struct vps *vps, struct sps *sps,
                                         struct pps *pps, u32 sliceSize,

...
#define X265_CODING_TYPE(x)                                                    \
    (IS_X265_TYPE_I(x) ? VCENC_INTRA_FRAME :                                   \
                         (IS_X265_TYPE_B(x) ? VCENC_BIDIR_PREDICTED_FRAME :    \
...
#define CLIP_DURATION_FIX8(f)                                                  \
    CLIP3(MIN_FRAME_DURATION_FIX8, MAX_FRAME_DURATION_FIX8, f)
...
struct Lowres {
    ...
    //    be part of a real scenecut. bool   bKeyframe; bool bLastMiniGopBFrame;
    //
    //    double ipCostRatio;
    //
    ...
    //    sum(pixel^2) - sum(pixel)^2 for entire frame uint64_t  wp_sum[3];
...
bool AddPictureToLookahead(VCEncLookahead *lookahead, const VCEncIn *pEncIn,
                           VCEncOut *pEncOut);

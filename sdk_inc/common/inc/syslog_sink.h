...
/*
 *   module_name :          "VCD" "VCE" "BIGSEA" "F1" "F2" "F3" "F4" or some
 * others you like .. device_id :         ASIC board device id
 */
 ...
 *char*module_name
 *
 * eg.FB_SYSLOG(0, pid, SYSLOG_SINK_LEV_ERROR,"VCD","in %s : %d, pid = %d
 *SYSLOG_SINK_LEV_ERROR\n",__func__,__LINE__,pid);
 * Trans[xx][pid][module_name]......
 * eg. * Trans[00][2024][VCD]......
 *
 *kill -USR2(-USR1) pid can add/reduce syslog by changing syslog_sink_threshold
 *syslog_sink_threshold default value is SYSLOG_SINK_LEV_DEBUG, which means on
 *output SYSLOG_SINK_LEV_DEBUG level if add syslog, kill -USR1 pid, otherwise
 *kill -USR2 pid
 *
 *
 */
...
#define FB_SYSLOG(header, level, fmt, arg...)                                  \
    do {                                                                       \
        syslog(LOG_USER | level, "Trans[%02d][%ld][%s]" fmt,                   \
               header ? ((LOG_INFO_HEADER *)header)->device_id : -1,           \
               (long int)gettid(),                                             \
               header ? ((LOG_INFO_HEADER *)header)->module_name : NULL,       \
               ##arg);                                                         \
    } while (0);
#else
#if 0
#define FB_SYSLOG(header, level, fmt, arg...)                                  \
    do {                                                                       \
        ...
        if (level <= syslog_sink_threshold) {                                  \
            fprintf(flog, "Trans[%02d][%d][%ld][L%d][%s]" fmt, device_id, pid, \
                    tid, level, module_name, ##arg);                           \
        }                                                                      \
        if (level == SYSLOG_SINK_LEV_ERROR) {                                  \
            fprintf(stderr, "Trans[%02d][%d][%ld][%s]" fmt, device_id, pid,    \
                    tid, module_name, ##arg);                                  \
        }                                                                      \
        if (level == SYSLOG_SINK_LEV_INFO) {                                   \
            fprintf(stdout, "Trans[%02d][%d][%ld][%s]" fmt, device_id, pid,    \
                    tid, module_name, ##arg);                                  \
        }                                                                      \
    } while (0);
#endif
